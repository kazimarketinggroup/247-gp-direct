...
import { familyCoverPage } from "@/lib/site";

export const metadata: Metadata = {
  title: "Cover for Your Family — 247 GP Direct",
  description:
    "One family membership designed to support the people you look after, including children where appropriate.",
};

export default function FamilyCoverPage() {
  return (
    <>
      <SilkHero
        image="/images/family-cover/hero-curve.png"
        label={familyCoverPage.label}
        title={familyCoverPage.title}
        intro={familyCoverPage.intro}
      />
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        ...
        <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,22rem)] lg:gap-12">
          <div className="flex flex-col gap-10 sm:gap-12">
            {familyCoverPage.blocks.map((block) => (
              <div key={block.title}>
